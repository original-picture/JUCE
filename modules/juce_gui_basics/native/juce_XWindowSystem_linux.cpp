...
void XWindowSystem::handleConfigureNotifyEvent (LinuxComponentPeer* peer, XConfigureEvent& confEvent) const
{
    ...
    if (confEvent.window == windowH && confEvent.above != 0 && isFrontWindow (windowH))
        peer->handleBroughtToFront(); // this doesn't always (ever?) get triggered when a window being brought to front
}
...
void XWindowSystem::handleClientMessageEvent (LinuxComponentPeer* peer, XClientMessageEvent& clientMsg, XEvent& event) const
{
    if (clientMsg.message_type == atoms.protocols && clientMsg.format == 32)
    {
        ...
        else if (atom == atoms.protocolList [XWindowSystemUtilities::Atoms::TAKE_FOCUS])
        {
            ...
            peer->handleBroughtToFront(); // the call to handleBroughtToFront in handleConfigureNotifyEvent doesn't always get triggered,
                                          // so I needed to find another way to detect when a window is brought to the front
                                          // in my testing, WM_TAKE_FOCUS is only generated when a window is brought to the front
                                          // (it gets generated by calls to toFront and ComponentPeer::toFront, [even if takeKeyboardFocus is false], but not grabFocus)
                                          // however, I've only tested this on ubuntu, so take all of these findings with a grain of salt
                                          // this might cause bugs in the form of erroneous calls to handleBroughtToFront on other distros that use different window managers,
